                ...
                {filteredItems.map((item, index) => (
                  ...
                  >
                    <div className="relative h-48 md:h-56 overflow-hidden shrink-0">
                      ...
                      {/* Deal Tag */}
                      {item.type === 'deal' && (
                        <div className="absolute top-3 left-3 md:top-4 md:left-4 bg-linear-to-r from-yellow-500 to-amber-500 text-black px-3 py-1.5 md:px-4 md:py-2 rounded-xl md:rounded-2xl text-xs font-bold shadow-lg flex items-center gap-1">
                          <Zap className="w-3 h-3" />
                          DEAL
                        </div>
                      )}
                      {/* Dietary Tags */}
                      <div className="absolute bottom-3 left-3 md:bottom-4 md:left-4 flex gap-1 md:gap-2">
                        {item.dietaryInfo?.isSpicy && (
                          <span className="bg-linear-to-r from-orange-500 to-red-500 text-white px-2 py-1 md:px-3 md:py-1.5 rounded-full text-xs font-medium flex items-center gap-1 shadow-md">
                            <Flame className="w-3 h-3" /> Spicy
                          </span>
                        )}
                        {item.dietaryInfo?.isVegan && (
                          <span className="bg-linear-to-r from-emerald-500 to-green-500 text-white px-2 py-1 md:px-3 md:py-1.5 rounded-full text-xs font-medium flex items-center gap-1 shadow-md">
                    ...
                    <div className="p-4 md:p-6 flex flex-col grow">
                      <div className="flex justify-between items-start mb-3 md:mb-4">
                        <h3 className="text-lg md:text-xl font-bold text-gray-900 line-clamp-1 group-hover:text-red-600 transition-colors">
                          {item.name}
                        </h3>
                        <div className="flex items-center shrink-0 bg-amber-50 px-1.5 py-0.5 md:px-2 md:py-1 rounded-full">
                          <Star className="w-3 h-3 md:w-4 md:h-4 text-amber-500 fill-amber-500" />
                          <span className="ml-0.5 md:ml-1 text-xs md:text-sm font-semibold text-amber-700">
                            4.5
                          </span>
                        </div>
                      </div>

                      <p className="text-gray-600 text-xs md:text-sm mb-4 md:mb-6 line-clamp-2 grow leading-relaxed">
                        {item.description ||
                          (item.type === 'deal'
                            ? 'A money-saving delicious combo!'
                            : 'Freshly prepared for you.')}
                      </p>

                      <div className="flex justify-between items-center mt-auto pt-3 md:pt-4 border-t border-gray-100">
                        <div className="text-xs md:text-sm text-gray-500 font-medium capitalize">
